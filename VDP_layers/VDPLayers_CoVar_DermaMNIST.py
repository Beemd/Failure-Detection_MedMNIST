...
class VDP_Conv2D(nn.Module):
    ...
    def __init__(self, in_channels, out_channels,
        ...
        nn.init.uniform_(self.sigma_weight, a=weight_args.conv_input_sigma_min,
                         b=weight_args.conv_input_sigma_max)
...
#             muh_w = self.mean.weight
#             print('layer mean and input sigma shapes: ', muh_w.shape, sigma.shape)
            muhT_sigmab_mu = torch.matmul(muh_w, sigma)
            muhT_sigmab_mu = torch.matmul(muhT_sigmab_mu,  muh_w.permute([0, 2, 1]))
